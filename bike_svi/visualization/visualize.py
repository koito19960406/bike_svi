...
class DataVizExpolorer:
    def __init__(self, input_folder, output_folder, variable_to_plot):
        ...
        self.count_df = pd.read_csv(os.path.join(self.input_folder, "processed/cities/London/all_var_joined.csv"))
        self.count_station = pd.read_csv(os.path.join(self.input_folder,"external/cities/London/count_station.csv"))
        self.borough_gdf = gpd.read_file(os.path.join(self.input_folder,"external/cities/London/control_variables/statistical-gis-boundaries-london/ESRI/London_Borough_Excluding_MHW.shp")).\
            to_crs(4326)
    ...
    def spatial_join(self):
        ...
        self.count_gdf = pd.merge(self.count_df, self.count_station, on = "count_point_id", how = "left")
        self.count_gdf = gpd.GeoDataFrame(self.count_gdf, geometry=gpd.points_from_xy(self.count_gdf.longitude, self.count_gdf.latitude))
        # spatial join
    ...
    def prep_data(self):
        ...
        self.borough_count_df = pd.DataFrame(self.borough_count_gdf.drop(columns='geometry'))
        self.borough_count_df = self.borough_count_df.groupby(["NAME","year"])[self.variable_to_plot].agg("mean").reset_index()
        self.borough_count_df = (self.borough_count_df.set_index(['NAME', 'year']).
                            reindex(pd.MultiIndex.from_tuples(itertools.product(self.borough_count_df.NAME.unique(), self.borough_count_df.year.unique()))).
                            reset_index().
    ...
    def plot(self):
        ...
        variable_to_plot = variable_to_plot.values.reshape(len(borough_name.unique()), len(year.unique()), order="F")
        ...
        fig.text(0.02, 0.5, 'Boroughs', ha="center", va="center", rotation="vertical", fontsize=14)

        fig.suptitle(f"Changes of {self.variable_to_plot} between 2008 and 2020 by boroughs", fontsize=20, y=0.95)

        # save
        fig.set_facecolor("white")
        fig.savefig(os.path.join(self.output_folder, f"{self.variable_to_plot}_heatmap.png"), dpi=300)
        plt.close('all')
...
if __name__ == "__main__":
    ...
    variable_to_plot_list = ["pedal_cycles","vegetation", "sky", "building", "sidewalk"]
