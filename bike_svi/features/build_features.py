...
def main(dir_input, dir_output):
        ...
        if df_combined.empty:
            ...
            if "year" not in df.columns:
                df_combined = pd.merge(
                    df_combined, df, on=["count_point_id"], how="left"
                )
            else:
                df["year"] = df["year"].astype(int).astype(str)
                if "month" not in df.columns:
                    df_combined = pd.merge(
                        df_combined, df, on=["count_point_id", "year"], how="left"
                    )
                else:
                    ...
                    df_combined["year_x"] = (
                        df_combined["year_x"].astype(int).astype(str)
                    )
                    df_combined["year_y"] = (
                        df_combined["year_y"].astype(int).astype(str)
                    )
                    df_combined["month_x"] = (
                        df_combined["month_x"].astype(int).astype(str)
                    )
                    df_combined["month_y"] = (
                        df_combined["month_y"].astype(int).astype(str)
                    )

                    # Applying the custom month difference function
                    df_combined = df_combined[
                        df_combined.apply(month_difference, axis=1)
                    ]
                    ...
                    df_combined = df_combined.rename(
                        columns={"year_x": "year", "month_x": "month"}
                    )

    # After the loop, you can aggregate by the original year and month
    df_combined = (
        df_combined.groupby(["count_point_id", "year", "month"])
        .agg("mean")
        .reset_index()
    )
    ...
    df_combined["slope_binary_80percent"] = np.where(
        df_combined["slope"] > df_combined["slope"].quantile(0.80), 1, 0
    )
    df_combined["slope_binary"] = np.where(
        df_combined["slope"] > df_combined["slope"].quantile(0.7), 1, 0
    )
    df_combined["slope_binary_60percent"] = np.where(
        df_combined["slope"] > df_combined["slope"].quantile(0.60), 1, 0
    )
    ...
    df_combined["ss_visual_complexity"] = np.where(
        df_combined["ss_visual_complexity"] > 100,
        100,
        df_combined["ss_visual_complexity"],
    )
    df_combined["ss_sidewalk_binary"] = np.where(
        df_combined["ss_sidewalk"] > df_combined["ss_sidewalk"].quantile(0.7), 1, 0
    )
    # add 60% and 80% percentile binary columns for ss_sidewalk
    df_combined["ss_sidewalk_binary_80percent"] = np.where(
        df_combined["ss_sidewalk"] > df_combined["ss_sidewalk"].quantile(0.80), 1, 0
    )
    df_combined["ss_sidewalk_binary_60percent"] = np.where(
        df_combined["ss_sidewalk"] > df_combined["ss_sidewalk"].quantile(0.60), 1, 0
    )
    df_combined["ss_pedestrian_area_binary"] = np.where(
        df_combined["ss_pedestrian_area"] > 0, 1, 0
    )
    ...
    df_combined["ss_street_light_binary"] = np.where(
        df_combined["ss_street_light"] > 0, 1, 0
    )
    df_combined["ss_guard_rail_binary"] = np.where(
        df_combined["ss_guard_rail"] > 0, 1, 0
    )
    ...
    df_combined["ss_vegetation_binary_80percent"] = np.where(
        df_combined["ss_vegetation"] > df_combined["ss_vegetation"].quantile(0.80), 1, 0
    )
    df_combined["ss_vegetation_binary"] = np.where(
        df_combined["ss_vegetation"] > df_combined["ss_vegetation"].quantile(0.7), 1, 0
    )
    df_combined["ss_vegetation_binary_60percent"] = np.where(
        df_combined["ss_vegetation"] > df_combined["ss_vegetation"].quantile(0.60), 1, 0
    )
    ...
    df_combined[new_columns[0]] = (
        df_combined["ss_fence"]
        + df_combined["ss_barrier"]
        + df_combined["ss_wall"]
        + df_combined["ss_bridge"]
        + df_combined["ss_building"]
        + df_combined["ss_tunnel"]
    )
    df_combined[new_columns[1]] = (
        df_combined["ss_crosswalk_plain"]
        + df_combined["ss_rail_track"]
        + df_combined["ss_road"]
        + df_combined["ss_service_lane"]
    )
    df_combined[new_columns[2]] = (
        df_combined["ss_lane_marking_crosswalk"]
        + df_combined["ss_lane_marking_general"]
    )
    df_combined[new_columns[3]] = (
        df_combined["ss_mountain"]
        + df_combined["ss_sand"]
        + df_combined["ss_snow"]
        + df_combined["ss_terrain"]
        + df_combined["ss_water"]
    )
    df_combined[new_columns[4]] = (
        df_combined["ss_banner"]
        + df_combined["ss_billboard"]
        + df_combined["ss_catch_basin"]
        + df_combined["ss_cctv_camera"]
        + df_combined["ss_fire_hydrant"]
        + df_combined["ss_junction_box"]
        + df_combined["ss_mailbox"]
        + df_combined["ss_manhole"]
        + df_combined["ss_phone_booth"]
        + df_combined["ss_pole"]
        + df_combined["ss_traffic_sign_frame"]
        + df_combined["ss_utility_pole"]
        + df_combined["ss_traffic_light"]
        + df_combined["ss_traffic_sign_back"]
        + df_combined["ss_traffic_sign_front"]
        + df_combined["ss_trash_can"]
    )
    df_combined[new_columns[5]] = df_combined["od_person"]
    df_combined[new_columns[6]] = (
        df_combined["od_bicyclist"] + df_combined["od_bicycle"]
    )
    df_combined[new_columns[7]] = (
        df_combined["od_bus"]
        + df_combined["od_car"]
        + df_combined["od_caravan"]
        + df_combined["od_motorcycle"]
        + df_combined["od_truck"]
        + df_combined["od_other_vehicle"]
        + df_combined["od_trailer"]
        + df_combined["od_on_rails"]
        + df_combined["od_wheeled_slow"]
        + df_combined["od_ego_vehicle"]
    )
    df_combined[new_columns[8]] = (
        df_combined["od_bird"] + df_combined["od_ground_animal"]
    )
    ...
    cols_to_keep = new_columns + [
        "ss_visual_complexity",
        "ss_vegetation",
        "ss_bike_lane",
        "ss_bike_rack",
        "ss_curb",
    ...
    ss_od_columns_to_drop = [
        col
        for col in df_combined.columns
        if col.startswith("ss_") or col.startswith("od_")
    ]
    # remove columns that contains "binary" in their names
    ss_od_columns_to_drop = [
        col
        for col in ss_od_columns_to_drop
        if col not in cols_to_keep and "binary" not in col
    ]
    ...
    df_combined = df_combined[
        (df_combined["year"] >= "2008") & (df_combined["year"] <= "2023")
    ]

    # Find columns where maximum value is greater than 100
    max_over_x_cols = df_combined.columns[
        (df_combined.max().astype(float) > 100)
        & (~df_combined.columns.isin(["count_point_id", "count", "year"]))
    ]
    ...
    df_combined = df_combined[
        ["count", "count_log"] + [col for col in df_combined.columns if col != "count"]
    ]
