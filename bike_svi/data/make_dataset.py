...
def main(gsv_api_key, log_path, dir_input, dir_temp, dir_output, city_name):
    ...
    pids = pd.read_csv(dir_temp / "gsv_pids.csv")[["panoid", "year", "month", "lat", "lon"]]
    ...
    for years in pids_pixel_ratios.year.unique():
        ...
        for month in pids_pixel_ratios_year.month.unique():
            pids_pixel_ratios_year_month = pids_pixel_ratios_year[pids_pixel_ratios_year.month == month]
            pids_pixel_ratios_year_month = gpd.sjoin(count_station, pids_pixel_ratios_year_month, how="left", op="intersects")
            # aggregate all the columns except for count_point_id and year
            pids_pixel_ratios_year_month = pids_pixel_ratios_year_month.groupby(["count_point_id", "year", "month"]).agg("mean").reset_index()
            # drop latitude, longitude, lat, lon, and index_right
            pids_pixel_ratios_year_month.drop(columns=["latitude", "longitude", "lat", "lon", "index_right"], inplace=True)
    ...
    pids_pixel_ratios_all_year_month.columns = ["count_point_id", "year", "month"] + ["ss_" + col.lower().\
        replace(" - ", "_").replace(" ", "_").replace("(", "").replace(")", "") for col in pids_pixel_ratios_all_year_month.columns[3:]]
    ...
    for years in pids_label_counts.year.unique():
        ...
        for month in pids_label_counts_year.month.unique():
            pids_label_counts_year_month = pids_label_counts_year[pids_label_counts_year.month == month]
            pids_label_counts_year_month = gpd.sjoin(count_station, pids_label_counts_year_month, how="left", op="intersects")
            # aggregate all the columns except for count_point_id and year
            pids_label_counts_year_month = pids_label_counts_year_month.groupby(["count_point_id", "year", "month"]).agg("mean").reset_index()
            # drop latitude, longitude, lat, lon, and index_right
            pids_label_counts_year_month.drop(columns=["latitude", "longitude", "lat", "lon", "index_right"], inplace=True)
    ...
    # add prefix to column names
    pids_label_counts_all_year_month.columns = ["count_point_id", "year", "month"] + ["od_" + col.lower().\
        replace(" - ", "_").replace(" ", "_").replace("(", "").replace(")", "") for col in pids_label_counts_all_year_month.columns[3:]]
    
    # save pids_pixel_ratios and pids_label_counts
    pids_pixel_ratios_all_year_month.to_csv(dir_output / "count_pixel_ratios.csv", index=False)
    pids_label_counts_all_year_month.to_csv(dir_output / "count_label_counts.csv", index=False)
