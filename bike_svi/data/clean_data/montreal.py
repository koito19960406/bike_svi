...
class MontrealDataCleaner(BaseDataCleaner):
    ...
    def read_xml(self, path_input, interest_group: list[str], interest_col: str, condition: Callable, yield_data: Callable):
        # Define namespace dictionary
        namespaces = {'generic': 'http://www.SDMX.org/resources/SDMXML/schemas/v2_0/generic'}

        def iterparse_large_xml(file_path):
            context = etree.iterparse(file_path, events=('end',), tag='{http://www.SDMX.org/resources/SDMXML/schemas/v2_0/generic}Series')
            
            # Create a manual TQDM instance for yielding progress
            pbar = tqdm(total=None, desc='Yielding data', unit='series')
            
            for event, elem in context:
                values = {child.get('concept'): child.get('value') for child in elem.findall('generic:SeriesKey/generic:Value', namespaces=namespaces)}
                
                obs_value_elem = elem.find('generic:Obs/generic:ObsValue', namespaces=namespaces)
    ...
    def standardize_census_data(self, 
                                df, 
                                year, 
                                census_id: str, 
                                interest_col: str, 
        ...
        # rename columns
        df = df.rename({census_id: "census_id", interest_col: "interest_col", value_col: "value_col"}, axis=1)
        # drop other columns
        ...
        col_mapper = {value: key for key, values in col_mapper.items() for value in values}
        ...
        # pivot table
        df = df.pivot(index="census_id", columns="interest_col", values="value_col").reset_index()
        # add year column
        df["year"] = year
    ...
    def load_age_data(self, path_input, year):
        ...
        # set variables
        interest_group = {
            2006: ["1", "2", "8", "14", "20", "26", "32", "38", "44", "50", "56", "62", "68", "74", "80", "86", "92", "98", "104", "110", "116", "122"],
            2011: ["1", "3", "9", "15", "22", "28", "35", "41", "48", "54", "61", "67", "74", "80", "87", "93", "99", "106", "112", "118", "124", "130"],
            2016: ["Total - Age", "0 to 4 years", "5 to 9 years", "10 to 14 years", "15 to 19 years", "20 to 24 years", "25 to 29 years", "30 to 34 years", "35 to 39 years", "40 to 44 years", "45 to 49 years", "50 to 54 years", "55 to 59 years", "60 to 64 years", "65 to 69 years", "70 to 74 years", "75 to 79 years", "80 to 84 years", "85 to 89 years", "90 to 94 years", "95 to 99 years", "100 years and over"],
            2021: ["Total - Age groups of the population - 100% data", "    0 to 4 years", "    5 to 9 years", "    10 to 14 years", "    15 to 19 years", "    20 to 24 years", "    25 to 29 years", "    30 to 34 years", "    35 to 39 years", "    40 to 44 years", "    45 to 49 years", "    50 to 54 years", "    55 to 59 years", "    60 to 64 years", "    65 to 69 years", "    70 to 74 years", "    75 to 79 years", "    80 to 84 years", "    85 to 89 years", "    90 to 94 years", "    95 to 99 years", "    100 years and over"]
        }
        ...
        col_mapper = {
            ...
            2011: {
                ...
                "age_60_90": ["80", "87", "93", "99", "106", "112", "118", "124", "130"]
            },
            2016: {
                "total": ["Total - Age"],
                "age_0_19": ["0 to 4 years", "5 to 9 years", "10 to 14 years", "15 to 19 years"],
                "age_20_39": ["20 to 24 years", "25 to 29 years", "30 to 34 years", "35 to 39 years"],
                "age_40_59": ["40 to 44 years", "45 to 49 years", "50 to 54 years", "55 to 59 years"],
                "age_60_90": ["60 to 64 years", "65 to 69 years", "70 to 74 years", "75 to 79 years", "80 to 84 years", "85 to 89 years", "90 to 94 years", "95 to 99 years", "100 years and over"]
            },
            2021: {
        ...
        if (year == 2006) | (year == 2011):            
            def age_condition_func(values, obs_value, interest_group, interest_col):
                return (values.get('GEO', None) is not None and len(values.get('GEO', None)) == 11 and
                        values.get('Age', values.get(interest_col)) in interest_group and
                        values.get('Sex', None) == "1" and
                        obs_value is not None)
            def yield_func(values, obs_value, interest_col):
                return {'GEO': values['GEO'], interest_col: values.get('Age', values.get('AGE')), 'OBS_VALUE': obs_value}

            # read the xml file
            df = self.read_xml(path_input, interest_group[year], interest_col[year], condition=age_condition_func, yield_data=yield_func)
            # standardize census data
            df = self.standardize_census_data(df, year, census_id[year], interest_col[year], value_col[year], col_mapper[year], custom_clean_func=custom_clean_func)
            ...
            if year == 2016:
                ...
                df = self.standardize_census_data(df, year, census_id[year], interest_col[year], value_col[year], col_mapper[year])
        # save the data
    ...
    def load_income_data(self, path_input, year):
        ...
        col_mapper = {
            ...
            2016: {
                "average_income": ["After-tax income"]
            },
            2021: {
                "average_income": ["After-tax income"]
            }
        ...
        if (year == 2006):            
            def condition_func(values, obs_value, interest_group, interest_col):
                return (values.get('GEO', None) is not None and len(values.get('GEO', None)) == 11 and
                        values.get(interest_col) in interest_group and
                        values.get("Age") == "1" and
                        values.get('Sex', None) == "1" and
                        obs_value is not None)
                
            def yield_func(values, obs_value, interest_col):
                return {'GEO': values['GEO'], interest_col: values[interest_col], 'OBS_VALUE': obs_value}
            ...
            def custom_clean_func(df):
                ...
                return df
            # read the xml file
            df = self.read_xml(path_input, interest_group[year], interest_col[year], condition=condition_func, yield_data=yield_func)
            # standardize census data
            df = self.standardize_census_data(df, year, census_id[year], interest_col[year], value_col[year], col_mapper[year], custom_clean_func=custom_clean_func)
    ...
    def load_housing_data(self, path_input, year):
        ...
        col_mapper = {
            ...
            2016: {
                "housing_price": ["Median value of dwellings $"]
            },
            2021: {
                "housing_price": ["Median value of dwelling $"]
            }
        }
        ...
        if (year == 2006):            
            def condition_func(values, obs_value, interest_group, interest_col):
                return (values.get('GEO', None) is not None and len(values.get('GEO', None)) == 11 and
                        values.get(interest_col) in interest_group and
                        values.get("BedRm") == "1" and
                        values.get('DType', None) == "1" and
                        obs_value is not None)
    ...
    def load_data(self):
        ...
        dissem_area_2006 = gpd.read_file(Path(self.dir_input) / "control_variables/admin_boundary/gda_000b06a_e/gda_000b06a_e.shp").to_crs("EPSG:4326")
        dissem_area_2011 = gpd.read_file(Path(self.dir_input) / "control_variables/admin_boundary/gda_000b11a_e/gda_000b11a_e.shp").to_crs("EPSG:4326")
        dissem_area_2016 = gpd.read_file(Path(self.dir_input) / "control_variables/admin_boundary/lda_000b16a_e/lda_000b16a_e.shp").to_crs("EPSG:4326")
        dissem_area_2021 = gpd.read_file(Path(self.dir_input) / "control_variables/admin_boundary/lda_000b21a_e/lda_000b21a_e.shp").to_crs("EPSG:4326")
        census_tract_2016 = gpd.read_file(Path(self.dir_input) / "control_variables/admin_boundary/lct_000b16a_e/lct_000b16a_e.shp").to_crs("EPSG:4326")
        census_tract_2021 = gpd.read_file(Path(self.dir_input) / "control_variables/admin_boundary/lct_000b21a_e/lct_000b21a_e.shp").to_crs("EPSG:4326")
        census_subdivision_2016 = gpd.read_file(Path(self.dir_input) / "control_variables/admin_boundary/lcsd000b16a_e/lcsd000b16a_e.shp").to_crs("EPSG:4326")
        census_subdivision_2021 = gpd.read_file(Path(self.dir_input) / "control_variables/admin_boundary/lcsd000b21a_e/lcsd000b21a_e.shp").to_crs("EPSG:4326")

        # load age data
        age_2006 = self.load_age_data(Path(self.dir_input) / "control_variables/age/97-551-XCB2006006/Generic_97-551-XCB2006006.xml", 2006)
        age_2011 = self.load_age_data(Path(self.dir_input) / "control_variables/age/98-311-XCB2011018/Generic_98-311-XCB2011018.xml", 2011)
        age_2016 = self.load_age_data(Path(self.dir_input) / "control_variables/age/98-400-X2016003_ENG_CSV/98-400-X2016003_English_CSV_data.csv", 2016)
        age_2021 = self.load_age_data(Path(self.dir_input) / "control_variables/age/98-401-X2021007_eng_CSV/98-401-X2021007_English_CSV_data.csv", 2021)
        
        # load income (Average/median 2005 after-tax income $) data
        income_2006 = self.load_income_data(Path(self.dir_input) / "control_variables/deprivation/97-563-XCB2006072/Generic_97-563-XCB2006072.xml", 2006)
        income_2016 = self.load_income_data(Path(self.dir_input) / "control_variables/deprivation/98-400-X2016121_ENG_CSV/98-400-X2016121_English_CSV_data.csv", 2016)
        income_2021 = self.load_income_data(Path(self.dir_input) / "control_variables/deprivation/98100071-eng/98100071.csv", 2021)
        
        # load housing (Median value of dwelling $) data
        housing_2006 = self.load_housing_data(Path(self.dir_input) / "control_variables/housing/97-554-XCB2006040/Generic_97-554-XCB2006040.xml", 2006)
        housing_2016 = self.load_housing_data(Path(self.dir_input) / "control_variables/housing/98-400-X2016233_ENG_CSV/98-400-X2016233_English_CSV_data.csv", 2016)
        housing_2021 = self.load_housing_data(Path(self.dir_input) / "control_variables/housing/98100257-eng/98100257.csv", 2021)
        
        # load landuse data 
        landuse_2016 = gpd.read_file(Path(self.dir_input) / "control_variables/land_use/graffectations.geojson")
        
        # load slope data
        path_input = str(Path(self.dir_input) / "gis_variables/slope/cdem_dem_031H_tif/cdem_dem_031H.tif")
        ...
        self.census_boundaries = {
            "dissem_area_2006": {"data":dissem_area_2006,
                                "census_id": "DAUID"},
            "dissem_area_2011": {"data":dissem_area_2011,
                                "census_id": "DAUID"},
            "dissem_area_2016": {"data":dissem_area_2016,
                                "census_id": "DAUID"},
            "dissem_area_2021": {"data":dissem_area_2021,
                                "census_id": "DAUID"},
            "census_tract_2016" : {"data":census_tract_2016,
                                "census_id": "CTUID"},
            "census_tract_2021": {"data":census_tract_2021,
                                "census_id": "CTUID"},
            "census_subdivision_2016": {"data":census_subdivision_2016,
                                "census_id": "CSDUID"},
            "census_subdivision_2021": {"data":census_subdivision_2021,
                                "census_id": "CSDUID"}
        }
        
        self.age_data = {
            2006: {"data": age_2006,
                    "census_boundaries": "dissem_area_2006"},
            2011: {"data": age_2011,
                    "census_boundaries": "dissem_area_2011"},
            2016: {"data": age_2016,  
                    "census_boundaries": "dissem_area_2016"},
            2021: {"data": age_2021,
                    "census_boundaries": "census_tract_2021"}
        }

        self.income_data = {
            2006: {"data": income_2006,
                    "census_boundaries": "dissem_area_2006"},
            2016: {"data": income_2016,
                    "census_boundaries": "census_tract_2016"},
            2021: {"data": income_2021,
                    "census_boundaries": "census_tract_2021"}
        }
        
        self.housing_data = {
            2006: {"data": housing_2006,
                    "census_boundaries": "dissem_area_2006"},
            2016: {"data": housing_2016,
                    "census_boundaries": "census_subdivision_2016"},
            2021: {"data": housing_2021,
                    "census_boundaries": "census_subdivision_2021"}
        }
        
        self.landuse_data = {
            2016: {"data": landuse_2016,
    ...
    def clean_wealth_data(self):
        ...
        # save to csv
        self.count_income.to_csv(Path(self.dir_output) / "count_income.csv", index=False)
        pass
    ...
    def clean_landuse_data(self):
        ...
        self.count_land_use = self.count_land_use.groupby(["count_point_id", "year", "land_use"], as_index=False).\
            agg({"area": "sum"})
            
        # pivot table
        self.count_land_use = self.count_land_use.pivot_table(index=["count_point_id", "year"], columns="land_use", values="area").reset_index()
