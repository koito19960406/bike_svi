...
class LondonDataCleaner(BaseDataCleaner):
    ...
    def load_age_data(self):
        if os.path.exists(
            os.path.join(self.dir_input, "control_variables/age", "age.csv")
        ):
            age = pd.read_csv(
                os.path.join(self.dir_input, "control_variables/age", "age.csv")
            )
        ...
        def _preclean_age_data(file_path, year):
            ...
            if os.path.exists(
                os.path.join(os.path.dirname(file_path), f"age_{str(year)}.csv")
            ):
                age = pd.read_csv(
                    os.path.join(os.path.dirname(file_path), f"age_{str(year)}.csv")
                )
                ...
                if 2012 <= year <= 2020:
                    ...
                    age = pd.read_excel(
                        file_path, sheet_name=f"Mid-{str(year)} Persons", header=None
                    ).iloc[4:, :]
                    # set column 'names
                ...
                while age_counter <= 60:
                    if year != 2011:
                        if age_counter < 60:
                            ...
                            age[f"age_{str(age_counter)}_{str(age_counter+19)}"] = (
                                age.iloc[:, 2:22].sum(axis=1) / age["all_ages"] * 100
                            )
                            # drop columns
                            age.drop(age.iloc[:, 2:22], inplace=True, axis=1)
                        else:
                            # sum from 60 to 90+ (31 columns)
                            age[f"age_{str(age_counter)}_{str(age_counter+30)}"] = (
                                age.iloc[:, 2:33].sum(axis=1) / age["all_ages"] * 100
                            )
                            # drop columns
                        ...
                        if age_counter < 60:
                            ...
                            age[f"age_{str(age_counter)}_{str(age_counter+19)}"] = (
                                age.iloc[:, 2:6].sum(axis=1) / age["all_ages"] * 100
                            )
                            # drop columns
                            ...
                            age[f"age_{str(age_counter)}_{str(age_counter+30)}"] = (
                                age.iloc[:, 2:9].sum(axis=1) / age["all_ages"] * 100
                            )
                            # drop columns
        ...
        years_and_files = [
            (
                2020,
                "control_variables/age/sape23dt10amid2020coaunformattedsyoaestimateslondon.xlsx",
            ),
            (
                2019,
                "control_variables/age/SAPE22DT2-mid-2019-lsoa-syoa-estimates-unformatted.xlsx",
            ),
            (
                2018,
                "control_variables/age/SAPE21DT1a-mid-2018-on-2019-LA-lsoa-syoa-estimates-formatted.xlsx",
            ),
            (
                2017,
                "control_variables/age/SAPE20DT1-mid-2017-lsoa-syoa-estimates-formatted.XLS",
            ),
            (
                2016,
                "control_variables/age/SAPE20DT1-mid-2016-lsoa-syoa-estimates-formatted.xls",
            ),
            (
                2015,
                "control_variables/age/SAPE20DT1-mid-2015-lsoa-syoa-estimates-formatted.xls",
            ),
            (
                2014,
                "control_variables/age/SAPE20DT1-mid-2014-lsoa-syoa-estimates-formatted.xls",
            ),
            (
                2013,
                "control_variables/age/SAPE20DT1-mid-2013-lsoa-syoa-estimates-formatted.xls",
            ),
            (
                2012,
                "control_variables/age/SAPE20DT1-mid-2012-lsoa-syoa-estimates-formatted.xls",
            ),
            (2011, "control_variables/age/mid-2011-lsoa-quinary-estimates.xls"),
            (
                2010,
                "control_variables/age/SAPE8DT1b-LSOA-syoa-unformatted-persons-mid2007-to-mid2010.xls",
            ),
            (
                2009,
                "control_variables/age/SAPE8DT1b-LSOA-syoa-unformatted-persons-mid2007-to-mid2010.xls",
        ...
        with ThreadPoolExecutor() as executor:
            futures = {
                executor.submit(
                    _preclean_age_data, os.path.join(self.dir_input, file), year
                ): (year, file)
            ...
            age_dataframes = []
            for future in tqdm(
                as_completed(futures), total=len(futures), desc="Processing age files"
            ):
    ...
    def load_wealth_data(self):
        ...
        def _preclean_wealth_data(file_path, year, columns):
            # load clean data if there is
            if os.path.exists(
                os.path.join(os.path.dirname(file_path), f"deprivation_{str(year)}.csv")
            ):
                deprivation = pd.read_csv(
                    os.path.join(
                        os.path.dirname(file_path), f"deprivation_{str(year)}.csv"
                    )
                )
                return deprivation
            deprivation = pd.read_excel(file_path, sheet_name=f"IMD {str(year)}").iloc[
                :, columns
            ]
            # add year column
            ...
            deprivation.rename(
                columns={
                    deprivation.columns[0]: "census_id",
                    deprivation.columns[1]: f"IMD_score",
                },
                inplace=True,
            )
            # save to a csv file
        ...
        with ThreadPoolExecutor() as executor:
            futures = {
                executor.submit(
                    _preclean_wealth_data,
                    os.path.join(self.dir_input, file),
                    year,
                    columns,
                ): (year, file)
                for year, file, columns in years_and_files
            }
            ...
            for future in tqdm(
                ...
                total=len(futures),
                desc="Processing deprivation files",
            ):
                result = future.result()
        ...
        deprivation.to_csv(
            os.path.join(
                self.dir_input, "control_variables/deprivation", "deprivation.csv"
            ),
            index=False,
        )
    ...
    def load_housing_data(self):
        ...
        if os.path.exists(
            os.path.join(self.dir_input, "control_variables/housing/housing.csv")
        ):
            housing = pd.read_csv(
                os.path.join(self.dir_input, "control_variables/housing/housing.csv")
            )
        ...
        def _preclean_housing_data(file_path):
            housing_df = pd.read_excel(file_path, sheet_name="Data", header=None).iloc[
                5:, :
            ]
            # set column 'names
            ...
            housing_df.columns = [
                (
                    "housing_price" + re.search(r"[0-9]{4}", col).group()
                    if re.search(r"[0-9]{4}", col) is not None
                    else col
                )
                for col in housing_df.columns
            ]
            housing_df = pd.wide_to_long(
                housing_df, stubnames=["housing_price"], i="LSOA code", sep="", j="year"
            )
            housing_df["housing_price"] = housing_df["housing_price"].replace(
                ":", pd.NA
            )
            housing_df = (
                housing_df.groupby(["LSOA code", "year"])["housing_price"]
                .agg("mean")
                .reset_index()
            )
        ...
        housing = _preclean_housing_data(
            os.path.join(
                self.dir_input,
                "control_variables/housing/hpssadataset46medianpricepaidforresidentialpropertiesbylsoa.xls",
            )
        )
        # save it as csv
        housing.to_csv(
    ...
    def load_land_use_data(self):
        ...
        if os.path.exists(
            os.path.join(self.dir_input, "control_variables/land_use/land_use.csv")
        ):
        ...
        def _preclean_land_use_data(file_path):
            land_use_df = (
                pd.read_excel(file_path, sheet_name=0, header=[3, 4, 5])
                .iloc[4:, :]
            ...
            land_use_df = land_use_df.drop(
                land_use_df.filter(regex="Grand Total").columns, axis=1
            )
            # remove uncessary strings like 1_level_1 and Unnamed:
            land_use_df.columns = [
                re.sub("[0-9]+_level_[0-9]+", "", col)
                .replace("Unnamed: ", "")
                .replace("Total", "")
                .replace("  ", " ")
                .replace("Developed use", "")
                .replace("Non-developed use", "")
                .replace("Non-developed", "")
                .strip()
                for col in land_use_df.columns
            ]
            land_use_df.columns = [
                "drop" if len(col) == 0 else col for col in land_use_df.columns
            ]
            # drop unnecessary columns
            land_use_df = land_use_df.drop("drop", axis=1)
            # replace space, "and", and "," with "_"
            land_use_df.columns = [
                col.replace(" and ", "_").replace(", ", "_").replace(" ", "_").lower()
                for col in land_use_df.columns
            ]
            # add prefix ("lu_") to column names
            land_use_df.columns = [
                "lu_" + col if col != "lsoa_code" else col
                for col in land_use_df.columns
            ]
            # replace - with 0
            ...
            # rename lsoa code
            land_use_df.rename(
                columns={land_use_df.columns[0]: "census_id"}, inplace=True
            )
            # create new land use columns by aggregating the existing ones
            land_use_df["lu_residential_community"] = (
                land_use_df["lu_community_service"]
                + land_use_df["lu_residential"] / 100
            )
            land_use_df["lu_commerce_developed"] = (
                land_use_df["lu_industry_commerce"]
                + land_use_df["lu_transport_utilities"]
                + land_use_df["lu_unknown_developed_use"] / 100
            )
            land_use_df["lu_others"] = (
                land_use_df["lu_agriculture"]
                + land_use_df["lu_forest_open_land_water"]
                + land_use_df["lu_outdoor_recreation"]
                + land_use_df["lu_residential_gardens"]
                + land_use_df["lu_defence"]
                + land_use_df["lu_minerals_landfill"]
                + land_use_df["lu_undeveloped_land"]
                + land_use_df["lu_vacant"] / 100
            )
            # drop the original columns
            land_use_df = land_use_df.drop(
                [
                    "lu_community_service",
                    "lu_residential",
                    "lu_industry_commerce",
                    "lu_transport_utilities",
                    "lu_unknown_developed_use",
                    "lu_agriculture",
                    "lu_forest_open_land_water",
                    "lu_outdoor_recreation",
                    "lu_residential_gardens",
                    "lu_defence",
                    "lu_minerals_landfill",
                    "lu_undeveloped_land",
                    "lu_vacant",
                ],
                axis=1,
        ...
        land_use = _preclean_land_use_data(
            os.path.join(
                self.dir_input,
                "control_variables/land_use/Live_Tables_-_Land_Use_Stock_2022_-_LSOA.ods",
            )
        )
        # save it as csv
        land_use.to_csv(
    ...
    def load_data(self):
        # load lsoa data
        self.lsoa_gdf_2011 = self.load_lsoa(
            os.path.join(
                self.dir_input,
                "control_variables/statistical-gis-boundaries-london/ESRI/LSOA_2011_London_gen_MHW.shp",
            )
        )
        self.lsoa_gdf_2004 = self.load_lsoa(
            os.path.join(
                self.dir_input,
                "control_variables/statistical-gis-boundaries-london/ESRI/LSOA_2004_London_Low_Resolution.shp",
            )
        )
        # load age data
        ...
        path_input = str(
            Path(self.dir_input)
            / "gis_variables/slope/LIDAR_10m_DTM_Composite_2019/LIDAR_10m_DTM_Composite.tif"
        )
        ...
        self.census_boundaries = {
            "lsoa_gdf_2011": {"data": self.lsoa_gdf_2011, "census_id": "census_id"},
            "lsoa_gdf_2004": {"data": self.lsoa_gdf_2004, "census_id": "census_id"},
        }

        self.age_data = {-1: {"data": age, "census_boundaries": "lsoa_gdf_2011"}}

        self.wealth_data = {
            2010: {
                "data": deprivation[deprivation["year"] == 2010],
                "census_boundaries": "lsoa_gdf_2004",
            },
            2015: {
                "data": deprivation[deprivation["year"] == 2015],
                "census_boundaries": "lsoa_gdf_2011",
            },
            2019: {
                "data": deprivation[deprivation["year"] == 2019],
                "census_boundaries": "lsoa_gdf_2011",
            },
        }

        self.housing_data = {
            -1: {"data": housing, "census_boundaries": "lsoa_gdf_2011"}
        }

        self.landuse_data = {
            -1: {"data": land_use, "census_boundaries": "lsoa_gdf_2011"}
        }
    ...
    def clean_count_station_data(self):
        ...
        self.count_station = pd.read_csv(
            os.path.join(self.dir_input, "count_station.csv")
        )
        # only keep count_point_id, latitude, longitude
        self.count_station = self.count_station[
            ["count_point_id", "latitude", "longitude"]
        ]
        # read count_data.csv
        self.count_data = pd.read_csv(
            os.path.join(self.dir_input, "dft_rawcount_region_id_6.csv")
        )
        ...
        self.count_data = (
            self.count_data.groupby(["count_point_id", "count_date", "hour"])
            .agg({"count": "sum"})
            .reset_index()
        )
        ...
        self.count_data = (
            self.count_data.groupby(["count_point_id", "year", "month"])
            .agg({"count": "mean"})
            .reset_index()
        )
        ...
        self.count_station[["count_point_id", "latitude", "longitude"]].to_csv(
            Path(self.dir_input) / "count_station_clean.csv", index=False
        )
        # save df.count_station_year_month: count_point_id, year, month, count
        self.count_data[["count_point_id", "year", "month", "count"]].to_csv(
            Path(self.dir_output) / "count_station_year_month.csv", index=False
        )
        # join count_station_year_month with count_station
        self.count_station_year_month = self.count_data.merge(
    ...
    def join_count_station_with_census(self, data_name):
                ...
                for unique_year in unique_years:
                    count_census_year = gpd.sjoin_nearest(
                        count_station_year_month_utm[
                            count_station_year_month_utm["year"] == unique_year
                        ],
                        census_data_gdf_year_utm[
                            census_data_gdf_year_utm["year"] == unique_year
                        ],
                        how="left",
                        max_distance=10000,
                        lsuffix="count",
        ...
        if "year_count" in count_census.columns:
            ...
            for count_point_id in tqdm(
                count_census["count_point_id"].unique(),
                desc="Interpolating missing years",
            ):
                df_subset = count_census[
                    count_census["count_point_id"] == count_point_id
                ]
            ...
            count_census = count_census.drop_duplicates(
    ...
    def clean_wealth_data(self):
        if (Path(self.dir_output) / "count_deprivation.csv").exists():
            self.count_deprivation = pd.read_csv(
                Path(self.dir_output) / "count_deprivation.csv"
            )
            return
        ...
        # save to csv
        self.count_deprivation.to_csv(
            Path(self.dir_output) / "count_deprivation.csv", index=False
        )
        pass
