...
capture.output(model_ols_summary, file= paste0(root_dir, "/models/ols.txt"))

# normal poisson
model_poisson <- glm(pedal_cycles ~ ., family = poisson(), data = all_var)
model_poisson_summary <- summary(model_poisson)
capture.output(model_poisson_summary, file= paste0(root_dir, "/models/poisson.txt"))
...
capture.output(model_year_fe_ols_summary, file= paste0(root_dir, "/models/year_fe_ols.txt"))
...
capture.output(model_year_fe_poisson_summary, file= paste0(root_dir, "/models/year_fe_poisson.txt"))
