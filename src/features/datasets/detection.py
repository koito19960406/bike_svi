...
class DetectionInferDataset(Dataset):
    def __init__(self, img_dir, gsv_invalid_file, feature_extractor, object_detection_raw_file_folder):
        super().__init__()
        ...
        try:
            detected_image_file_set = set([file[:-4] for file in os.listdir(object_detection_raw_file_folder)])
        except:
            detected_image_file_set = None
        gsv_invalid_set = set(pd.read_csv(gsv_invalid_file)["file_name"].unique())
        ...
        def check_file(image_file):
            if image_file not in gsv_invalid_set:
                # run the following only if the file_name doesn't exist in the file yet
                if detected_image_file_set is not None:
                    # skip if it's already been sgemented or invalids
                    if image_file[:-4] not in detected_image_file_set:
        ...
        def parallelize_list(cpu_num, input_list, outer_func):
            ...
            output_list_temp = pool.map(outer_func, input_list_split)
            pool.close()
            pool.join()
            output_list = list(itertools.chain.from_iterable(output_list_temp))
